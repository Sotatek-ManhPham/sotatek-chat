...
int numberOfEmoji() {
  return unicodeMap.length;
}

String getEmojiByIndex(int index) {
  var keys = [];
  for (var key in unicodeMap.keys) {
    keys.add(key);
  }
  return emojify(keys[index]);
}

String emojify(String input) {
  final output = input.replaceAllMapped(new RegExp(r':\w+:'), (match) {
    if (unicodeMap[match.group(0)] != null) {
      return '' + unicodeMap[match.group(0)] + '';
    } else {
